...
final class ManageGroupRepository {
  ...
  void getGroupCapacity(@NonNull GroupId groupId, @NonNull Consumer<GroupCapacityResult> onGroupCapacityLoaded) {
    SimpleTask.run(SignalExecutors.BOUNDED, () -> {
      GroupTable.GroupRecord groupRecord = SignalDatabase.groups().getGroup(groupId).get();
  ...
  void setMentionSetting(@NonNull GroupId groupId, RecipientTable.MentionSetting mentionSetting) {
    SignalExecutors.BOUNDED.execute(() -> {
      RecipientId recipientId = Recipient.externalGroupExact(groupId).getId();
